...
<body>
	...
	<div>
		...
		<!-- -->
		<form:form modelAttribute="newReport" action="${saveReport}"
			method="post">
			<table>
				<tr>
					<th>Tasks</th>
				...
				<c:forEach items="${newReport.tasks}" varStatus="vs">
					<tr>
						...
						<td><form:input path="tasks[${vs.index}].task"
								placeholder="Task detail" /></td>
						<td><form:input path="tasks[${vs.index}].time"
								placeholder="How much time did you use" /></td>
						<td><form:input path="tasks[${vs.index}].status"
								placeholder="Status so far" /></td>
					</tr>
				</c:forEach>
				<%-- <tr>
         ...
         --%>
			...
			<p>
				Signed:
				<form:input width="50" type="text" path="sign" />
			</p>
			<p>
				Date:&nbsp;&nbsp;&nbsp;
				<form:input width="100px" type="date" path="date" />
			</p>
			...
			<c:if test="${role == 'ROLE_PRO' }">
				...
				<%-- <c:if test="${ newReport.grade == 0}"> --%>
					...
					<p>
						Grade:
						<form:input width="50" type="text" path="grade" />
					</p>
				<%-- </c:if> --%>
				...
				<%-- <c:if test="${empty newReport.comment}"> --%>
					...
					<p>Comments:
					<p>
						<form:textarea rows="20" cols="100" path="comment" />
					</p>
					<br />
				<%-- </c:if> --%>
